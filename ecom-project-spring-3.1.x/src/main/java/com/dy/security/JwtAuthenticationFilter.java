	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if (requestToken != null && requestToken.trim().startsWith("Bearer")) {
			...
			if (Username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
				...
				// vaildate
				UserDetails userDtails = this.userDetailsService1.loadUserByUsername(Username);

				if (this.jwtHelper.vaildateToken(jwtToken, userDtails)) {

					UsernamePasswordAuthenticationToken auth = new UsernamePasswordAuthenticationToken(userDtails, null,
